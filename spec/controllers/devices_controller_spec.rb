...
describe DevicesController do
  context "#update" do
    context "existing" do
      let!(:device) { create :device, physical_device_id: 'xxx',  ip: 123}
      before{ put :update, id: 'xxx', device: { physical_device_id: 'xxx', ip: 1234 } }
      it { expect(Device.last.ip).to eq(1234) }
    end
    context "non existed" do
      before{ put :update, id: 'xxx', device: { physical_device_id: 'xxxz', ip: 1234 } }
      it { expect(Device.last.physical_device_id).to eq('xxxz') }
    end
  end
end
...
#         expect_any_instance_of(Device).to receive(:update).with({ "these" => "params" })
